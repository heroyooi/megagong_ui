...
'			intGWidth2X = 1660 : intGHeight2X = 1260
	...
	Else
		l_LHF = "D"
	End If
	uservalues = "&uservalue0=" & APP_NO & "&uservalue1=" & l_LHF & "&uservalue2=" & CHR_CD & "&uservalue3=" & LEC_CD & "&uservalue4=P&uservalue5=0&uservalue8=R"
%>
...
<!--
	...
	<% End If %>
		try {
			...
			controller
				.on('pause', function(percent, position, duration) {
					controller.play();
				});
			controller.set_topmost(true);
			controller.set_jumpstep(0);
                    ...
                    <td style="text-align:center;background-color:#000;">
                        ...
                        <tr>
                            <td style="background-color:#000;">
								<iframe id="child" width="100%" height="<%= newGHeight - new_cutsize - new_childsize %>" style="margin:0;" src="http://v.kr.kollus.com/s?jwt=<%=tmp%>.<%=result%>&custom_key=<%=customKey%><%=uservalues%>&a&s=0" frameborder="0" allowfullscreen webkitallowfullscreen mozallowfullscreen></iframe>
                            </td>
                        </tr>
                        </table>
                        <!-- 영상 영역 끝 -->
