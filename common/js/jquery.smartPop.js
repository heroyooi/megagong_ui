...
;(function($) {
    ...
    $.smartPop = {
        ...
        resize : function() {
            ...
            this.resizeHeight(this.opts.height,this.opts.width);			
        },
        resizeHeight : function(h,w) {
            ...
            // 위치설정
//			alert($(".mbm_wrap").height());		
			innerH = $(".mbm_wrap").height();
			innerW = $(".mbm_wrap").width();
            if(this.opts.position == 'center') {
                var t;
				var w;
                t = (h < innerH) ? (innerH - h) / 2 : 10;
                w = (h < innerW) ? (innerW - w) / 2 : 10;
                $('#smartPop_container').css({ marginLeft: w + 'px' , marginTop: t + 'px' });
            } else {
