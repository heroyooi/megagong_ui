...
''''인증키 획득
''''리턴값 : 배열로 0:인증키, 1:인증문자열
''''사용법 : 페이지 파라미터로 위 두값을 넘긴 후 다음 페이지에서 아래의 함수를 호출
Function getAuth()
	Dim arRet(2), timestamp, timestamp2, dte2, dbg

	timestamp = year(now) & right("0" & month(now),2) & right("0" & day(now),2) & right("0" & hour(now),2) & right("0" & minute(now),2) & right("0" & second(now),2)

	arRet(0) = timestamp
	arRet(1) = SHA256("mcst" & timestamp)
...
''''인증키 체크
''''리턴값 : 인증 유효 여부 확인 불리언
''''사용법 : 위의 함수에서 넘긴 파리미터를 입력
Function chkAuth(ByVal dte, ByVal sstr)
	Dim ret, timestamp, timestamp2, dte2, dbg
	ret = False

	dte2 = DateAdd("h",-3,now())
	timestamp = year(now) & right("0" & month(now),2) & right("0" & day(now),2) & right("0" & hour(now),2) & right("0" & minute(now),2) & right("0" & second(now),2)
	timestamp2 = year(dte2) & right("0" & month(dte2),2) & right("0" & day(dte2),2) & right("0" & hour(dte2),2) & right("0" & minute(dte2),2) & right("0" & second(dte2),2)

	If dte <= timestamp And dte >= timestamp2 Then
		If SHA256("mcst" & dte) = sstr Then
			ret = true
		End If
	End If

'	ret = timestamp2 & " $$<br> " & dte & " &&<br> " & timestamp & " $$<br> " & sstr & " $$<br> " & SHA256("mcst" & dte)
	chkAuth = ret
End Function
%>
