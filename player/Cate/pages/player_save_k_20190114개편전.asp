...
'				Call adminlog("4 player_save_k.asp " & client_user_id & " : result : " & rStdSdt & "," & rStdEdt & "," & rSdown_cnt & "," & nCnt1)
				...
				If client_user_id = "lakemind" And rStdSdt <> "" And rStdEdt <> "" Then
					result(null)("expiration_date") = DateDiff("s", "1970-01-01 09:00:00", Now()+2)	' 만료될 시간의 unixtime stamp 지정
					'result(null)("expiration_date") = DateDiff("s", "1970-01-03 08:40:00", Now()+2)		' 테스트용
...
'						Call adminlog("34 player_save_k.asp " & client_user_id & " : result : " & rStdSdt & "," & rStdEdt & "," & rSdown_cnt)
					...
					Else 
						...
						result(null)("expiration_date") = DateDiff("s", "1970-01-01 09:00:00", Now()+2)	' 2일 추가 인증
