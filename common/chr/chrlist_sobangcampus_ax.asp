...
<%
    ...
    chr_para = "camp_cd=" & camp_cd & "&dom_cd=" & dom_cd & "&sch_cd=" & sch_cd & "&sch_yr=" & sch_yr & "&sch_mn=" & sch_mn & "&cls=" & cls 
    ...
    if isArray(ar_tab) Then
        ...
        strSql = strSql & "JOIN     MS_SUB_MAS WITH(NOLOCK) ON SM_SUB_CD = CMO_SUB_CD AND CMO_DOM_CD = SM_DOM_CD "
        ...
        strSql = ""
        strSql = strSql & "SELECT	LCODE = CMO_LEV_CODE, LNAME = CC_KOR_NM,CC_NUM1=CC_NUM1 "
        strSql = strSql & "FROM     MS_CHR_MAS_OFFLINE WITH(NOLOCK) "
        strSql = strSql & "JOIN     MS_SUB_MAS WITH(NOLOCK) ON SM_SUB_CD = CMO_SUB_CD  AND CMO_DOM_CD = SM_DOM_CD "
        ...
        if tno <> "" Then strSql = strSql & " AND   SG_TAB_FLG = '" & tno & "' "
        strSql = strSql & ") "
        strSql = strSql & "GROUP BY CMO_LEV_CODE, CC_KOR_NM,CC_NUM1 "
        strSql = strSql & "ORDER BY CC_NUM1 "
	    Set Rs = DBexec(strSql, "study")
	    If Not Rs.Eof Then
            ar_cls = Rs.GetRows()
	    End if
        ...
        ' if cook_id = "monome" OR cook_id = "jyleems" OR cook_id = "bingo" then            
                ...
                %>
			            <tr>
				            ...
				            <td class="info">
					            <span class="icon">
                                    <% if ing = "1" Then %>
                                        <img src="<%=img_main%>/c/sobang/academy/table_icon_2.gif" alt="마감임박" />
                                    <% Elseif ing = "2" Then %>
                                        <img src="<%=img_main%>/c/sobang/academy/table_icon_3.gif" alt="마감" />
                                    <% Elseif ing = "3" Then %>
                                        <img src="<%=img_main%>/c/sobang/academy/table_icon_4.gif" alt="접수대기" />
                                    <% Elseif ing = "4" Then %>
                                        <img src="<%=img_main%>/c/sobang/academy/table_icon_5.gif" alt="폐강" />
                                    <% Else %>
                                        <% if rto >= 1.0 Then %>
                                            <img src="<%=img_main%>/c/sobang/academy/table_icon_3.gif" alt="마감" />
                                        <% Elseif rto >= 0.7 And rto < 1.0 Then %>
                                            ...
                                            <% Elseif (tnt <= 79) And rnt < 9 Then %>
                                                <img src="<%=img_main%>/c/sobang/academy/table_icon_2.gif" alt="마감임박" />
                                            <% Else %>
                                                <img src="<%=img_main%>/c/sobang/academy/table_icon_1.gif" alt="접수증" />
                                            <% End if %>
                                        <% Else %>
                                            <img src="<%=img_main%>/c/sobang/academy/table_icon_1.gif" alt="접수증" />
                                        <% End if %>
						            ...
						            <% if inStr(tlg, "0") > 0 Then %>
                                        <img src="<%=img_main%>/c/sobang/academy/table_icon_6.gif" alt="일반공채" />
                                    <% End if %>
						            <% if inStr(tlg, "1") > 0 Then %>
                                        <img src="<%=img_main%>/c/sobang/academy/table_icon_7.gif" alt="구조구급특채" />
                                    <% End if %>
						            <% if inStr(tlg, "2") > 0 Then %>
                                        <img src="<%=img_main%>/c/sobang/academy/table_icon_8.gif" alt="소방학과특채" />
                                    <% End if %>
						            <% if inStr(tlg, "3") > 0 Then %>
                                        <img src="<%=img_main%>/c/sobang/academy/table_icon_9.gif" alt="특강" />
                                    <% End if %>
                                    <% '임시로 작업 - 확장 시 변경해야함 %>

                                    <% if clg = "301" Then %>
						                <img src="<%=img_main%>/c/sobang/academy/table_icon_10.gif" alt="기초입문" />
                                    <% End if %>
                                    <% if clg = "302" Then %>
						                <img src="<%=img_main%>/c/sobang/academy/table_icon_11.gif" alt="기본이론" />
                                    <% End if %>
                                    <% if clg = "303" Then %>
						                <img src="<%=img_main%>/c/sobang/academy/table_icon_12.gif" alt="심화이론" />
                                    <% End if %>
                                    <% if clg = "304" Then %>
						                <img src="<%=img_main%>/c/sobang/academy/table_icon_13.gif" alt="기출문풀" />
                                    <% End if %>
                                    <% if clg = "305" Then %>
						                <img src="<%=img_main%>/c/sobang/academy/table_icon_14.gif" alt="문제풀이" />
                                    <% End if %>
                                    <% if clg = "306" Then %>
						                <img src="<%=img_main%>/c/sobang/academy/table_icon_15.gif" alt="모의고사" />
                                    <% End if %>
                                    <% if clg = "307" Then %>
						                <img src="<%=img_main%>/c/sobang/academy/table_icon_16.gif" alt="파이널" />
                                    <% End if %>
                                    <% if clg = "308" Then %>
						                <img src="<%=img_main%>/c/sobang/academy/table_icon_17.gif" alt="전략특강" />
                                    <% End if %>
					            </span><br />
